...
function index()
	...
	if has_wifi then
			page = entry({"admin", "system", "ap_join"}, call("ap_join"), nil)
			page = entry({"admin", "system", "router"}, call("router"), nil)
			page = entry({"admin", "system", "ap_scan_cli"}, call("ap_scan_cli"), nil)
			page = entry({"admin", "system", "ap_scan_rep"}, call("ap_scan_rep"), nil)
			page.leaf = true
	end
...
function ap_scan_cli()
		
		luci.template.render("admin_system/ap_join")
end

function router()
		luci.sys.exec("/etc/init.d/router restart")
		luci.http.redirect(luci.dispatcher.build_url("admin/system/system_mode"))
end

function ap_scan_rep()
...
function ap_join()
	...
	local system_mode = param("system_mode")
	if system_mode then
		if system_mode == "router" then
			luci.sys.exec("/etc/init.d/system_mode restart router")
			luci.http.redirect(luci.dispatcher.build_url("admin/system/system_mode"))
			return
		end
		if system_mode == "bridge" then
			luci.sys.exec("/etc/init.d/system_mode restart bridge")
			luci.http.redirect(luci.dispatcher.build_url("admin/system/system_mode"))
			return
		end
		luci.sys.exec("rm /tmp/repeater;rm /tmp/ap_client;")
		luci.sys.exec("touch /tmp/%q" % system_mode)
	end
	...
	if dev and ssid then
		...
		if cancel then
			luci.http.redirect(luci.dispatcher.build_url("admin/network/ap_join?device=" .. dev))
		else
		  
			local cbi = require "luci.cbi"
			local tpl = require "luci.template"
			local map = luci.cbi.load("admin_system/ap_join")[1]
...
function action_flashops()
	...
	local function image_supported()
		...
		local imageSize = nixio.fs.stat(image_tmp).size
		local imgThreshold = tonumber(uci:get("system","sysupgrade","imgThreshold") or "6488064")
		if imageSize > imgThreshold then
			return false
		end
		return ( 0 == os.execute(
			". /lib/functions.sh; " ..
	...
	local function storage_size()
		return tonumber(uci:get("system","sysupgrade","imgThreshold") or "6488064")
