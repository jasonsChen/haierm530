...
if arg[1] == "mt7628.network1" then
	wdev:set("AccessPolicy0","0")
	wdev:set("AccessControlList0","")
elseif arg[1] == "mt7628.network2" then
	wdev:set("AccessPolicy1","0")
	wdev:set("AccessControlList1","")
end
...
if vendor == "ralink" then
	...
	ch:value("1","2412MHz (Channel 1)")
        ch:value("2","2417MHz (Channel 2)")
        ch:value("3","2422MHz (Channel 3)")
        ch:value("4","2427MHz (Channel 4)")
        ch:value("5","2432MHz (Channel 5)")
        ch:value("6","2437MHz (Channel 6)")
        ch:value("7","2442MHz (Channel 7)")
        ch:value("8","2447MHz (Channel 8)")
        ch:value("9","2452MHz (Channel 9)")
        ch:value("10","2457MHz (Channel 10)")
        ch:value("11","2462MHz (Channel 11)")
        ch:value("12","2467MHz (Channel 12)")
        ch:value("13","2472MHz (Channel 13)")
	end
	tp = s:taboption("general",ListValue,"txpower", translate("Transmit Power"))
	tp.rmempty = true
	tp.default = 100
	tp:value(20,translate("20%")) 
	tp:value(40,translate("40%")) 
	tp:value(60,translate("60%")) 
	tp:value(80,translate("80%")) 
	tp:value(100,translate("100%")) 

	bw = s:taboption("general", ListValue, "bw", translate("Band Width"))
	bw:value("0", translate("20MHz"))
        bw:value("1", translate("40MHz"),{wifimode="7"},{wifimode="9"})
	if band == "5G" then
...
--		local wh = 0
--		if arg[1] == "mt7628.network2" then
--			wh = 1
--		end
--		wh = "AccessPolicy" .. "" .. wh
		
--		local acp  = tostring(m.uci:get("wireless", section, wh))
--		if acp == "1" then
--			return "allow"
--		elseif acp == "2" then
--			return "deny"
...
--	end
	...
	function mp.write(self, section, value)
			local wh = 0
			if arg[1] == "mt7628.network2" then
				wh = 1
			end
			...
			if value == "allow" then
				wdev:set(wh, 1)
--				m.uci:set("wireless", section, wh, 1)
			elseif value == "deny" then
				wdev:set(wh, 2)
--				m.uci:set("wireless", section,wh, 2)
			else
				wdev:set(wh, 0)
			end
	...
	function ml.write(self, section, value)
		local _maclist = ""
		local i = 0
		local wh = 0
		if arg[1] == "mt7628.network2" then
		...
		wdev:set(wh, _maclist)
--		m.uci:set("wireless", section, wh, _maclist)
...
--	mode:value("sta-wds", "%s (%s)" % {translate("Client"), translate("WDS")})
	...
	wmm = s:taboption("general", ListValue, "wmm", translate("WMM Mode"))
        ...
        wmm:value("1","Enable")
        wmm.default = 1
	NoForwarding = s:taboption("general", Flag, "NoForwarding", translate("Isolate"))
	
	MaxStaNum = s:taboption("general", Value, "MaxStaNum", translate("Max Sta Number"))
	
	uplinkbw = s:taboption("general", Value, "uplinkbw", translate("Uplink bandwidth(KB/s)"))
	downlinkbw = s:taboption("general", Value, "downlinkbw", translate("Downloadlink bandwidth(KB/s)"))
end
